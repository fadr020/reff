...
def search_email(mail, subject, recipient, folder="inbox"):
    mail.select(folder)
    ...
    for email_id in reversed(email_ids):
        ...
        for response_part in msg_data:
            if isinstance(response_part, tuple):
                ...
                msg_to = msg["To"]
                if subject in msg_subject and recipient in msg_to:
...
def main():
    try:
        ...
        # Kamus untuk menyimpan pasangan (username, random_email)
        email_map = {}
        ...
        for index in range(1, desired_referrals + 1):
            for credential in credentials:
                ...
                try:
                    ...
                    email_map[imap_username] = new_email
                    ...
                    if request_otp(new_email):
                        ...
                        time.sleep(11)  # Tunggu beberapa detik agar email OTP dapat diterima

                        otp_subject = "Pixelverse Authorization"  # Sesuaikan dengan subjek email OTP yang diterima
                        inbox_body, inbox_timestamp = search_email(mail, otp_subject, new_email)
                        junk_body, junk_timestamp = search_email(mail, otp_subject, new_email, folder="junk")
